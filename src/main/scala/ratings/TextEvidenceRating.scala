...
object TextEvidenceRating extends Rating {
  ...
  val parentTitleWeight : Double = 0.6
...
class TextEvidenceRating extends JenaTitleAbstractDumpWrapper with RatingResult {
  ...
  def rateTypes(types: List[(String, List[String])], abstractsMapFuture: Future[List[(List[String], List[String])]], parentTitle: List[String]) : Future[List[(String, Double)]] = {
    val typeCounts = types map { typeTupel =>
      ...
      abstractsMapFuture.map(abstractsMap => {
        ...
        typeLabel -> (titleCount * titleWeight + abstractCount * abstractWeight + parentTitleCount * parentTitleWeight)
      })
    }
  ...
  def compute(title: String, uris: List[String], types: Map[String, Int])(implicit materializer: Materializer): Future[Map[String, Double]] = {
    ...
    rateTypes(normalizedTypes, abstractsMap, parentTitle)
      .map(_.toMap)
  }

  def getRating(result: WikiListResult)(implicit materializer: Materializer): Future[Map[String, Double]] = {
    compute(result.page.title, result.page.getEntityUris, result.types)
  }
}
