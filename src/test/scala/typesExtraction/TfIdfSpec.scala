...
class TfIdfSpec extends FlatSpec {
  ...
  val typesMap = Await.result(extractor.getTypesMap(list), 20 seconds)


  "A TfIdfWorker" should "get the tf-idf scores of the basketballers wikipage 1" in {
    val resultFut = tfIdf.getTfIdfScores(typesMap)
    Await.result(resultFut, 20 seconds)
  }

  it should "get the tf-idf scores of the basketballers wikipage 2" in {
    val resultFut = tfIdf.getTfIdfScores(typesMap)
    Await.result(resultFut, 20 seconds)
  }

  it should "get the tf-idf scores of the basketballers wikipage 3" in {
    val resultFut = tfIdf.getTfIdfScores(typesMap)
    Await.result(resultFut, 20 seconds)
  }
